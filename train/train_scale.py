...
OUT = '/tmp/matching/ce_scale'
BASE_MODEL = '/tmp/matching/ce_llm/best_llm'   # продолжаем от стадии 1 (llm_holdout 0.7309)
N_LLM_TRAIN = int(sys.argv[1]) if len(sys.argv) > 1 else 1_500_000
BATCH = 64
MAX_LENGTH = 384
LR_STAGE1 = 1e-5   # продолжение обучения, а не старт с нуля
LR_STAGE2 = 1e-5
REPLAY_LLM = 292_000
SEED = 42
...
llm_train = rest.sample(min(N_LLM_TRAIN, rest.height), seed=7)   # другой seed = новые пары
...
class LengthGroupedBatches(torch.utils.data.Sampler):
    """Батчи из примеров схожей длины: паддинг почти исчезает (~1.7x скорости).

    Внутри «мегабатча» сортируем по длине, режем на батчи, затем перемешиваем
    порядок батчей — случайность сохраняется, а padding падает.
    """

    def __init__(self, lengths, batch_size, shuffle, mega=50):
    ...
    def build(self):
        ...
        for i in range(0, len(idx), self.mega):
            ...
            batches += [chunk[j:j + self.bs].tolist() for j in range(0, len(chunk), self.bs)]
        if self.shuffle:
...
def loader(ds, shuffle, bs=BATCH):
    lengths = [len(TEXT[a]) + len(TEXT[b]) for a, b in zip(ds.a, ds.b)]
    sampler = LengthGroupedBatches(lengths, bs, shuffle)
    dl = DataLoader(ds, collate_fn=collate, num_workers=6, pin_memory=True,
                    batch_sampler=sampler)
    if not shuffle:
        # порядок фиксирован — запоминаем, чтобы вернуть скоры на исходные места
        dl.eval_order = np.concatenate([np.asarray(b) for b in sampler.build()])
    return dl
...
def scores_for(dl):
    ...
    order = getattr(dl, 'eval_order', None)
    if order is None:
        return scores
    restored = np.empty_like(scores)
    restored[order] = scores          # без этого скоры не совпадают с метками
    return restored
...
log('модель после первой стадии, до продолжения:')
evaluate('старт', 0)

# Стадия 1b: ещё 1.5M свежих LLM-пар. Вопрос эксперимента — упёрлись ли мы
# в потолок данных (0.7309) или он двигается с объёмом.
train(llm_dl, 1.0, LR_STAGE1, 'llm1b', 4000)

# Затем тот же replay, что дал лучший результат: human + LLM 1:1
replay = llm_train.head(REPLAY_LLM)
mix_a = [h1[i] for i in hum_tr] + replay['id1'].to_list()
mix_b = [h2[i] for i in hum_tr] + replay['id2'].to_list()
mix_y = np.concatenate([hy[hum_tr], replay['target'].to_numpy().astype(np.float32)])
log(f'смесь: human {len(hum_tr)} + LLM replay {replay.height} = {len(mix_a)} пар')
train(loader(Pairs(mix_a, mix_b, mix_y, True), True), 1.0, LR_STAGE2, 'replay', 2000)
log('ГОТОВО')
